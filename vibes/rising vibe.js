...
function onMessage (msg, intensity, messageparts=null) {
    ...
    if (getVibe(msg.author.id).find(vibe => vibe.vibetype == "rising vibe").intensity >= 20) { return }
    ...
    if ((0.25 * n_of_messages) > Math.random()) {
        const newintensity = Math.min(intensity + Math.floor(Math.random() * 4) + 3, 20);
        assignVibe(msg.author.id, newintensity, "rising vibe");
        messageSendBot(msg.channel.isThread() ? msg.channelId : null, `Oooops! <@${msg.author.id}>'s rising vibe increases it's intensity to ${newintensity}. Maybe ${getPronouns(msg.author.id, 'subject')} should try talking a little less~`);
        setUserVar(msg.author.id, "n_of_messages", 0);
    } else {
